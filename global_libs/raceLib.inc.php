...
class village{
			...
			foreach ($highlevels as &$value) {
			    ...
			    $this->output_buffer .= '<tr class="'.$row.'">
			    	<td width="8%" align="center">'.$i.'</td>
		      		<td width="63%" align="center"><b>'.$leader['0'].'</b>'.$challenge.'</td>
	...
	function squad(){
		...
		$this->output_buffer .= '<tr><td colspan="2" style="border-top:1px solid #000000;" align="center"><a href="?id='.$_GET['id'].'&i='.$_GET['i'].'&act=users&min='.($newminm).'">&laquo; Previous</a></td>
                                     <td colspan="2" style="border-top:1px solid #000000;" align="center"><a href="?id='.$_GET['id'].'&i='.$_GET['i'].'&act=users&min='.($newmini).'">Next &raquo;</a></td></tr>';
		$this->output_buffer .= '</table></div>';
	}
	...
	function view_profile(){
        ...
        if($char_data == '0 rows'){
			...
			//			Output Profile
			$this->output_buffer .= '<div align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  			<tr>
    		<td style="padding:10px;"><table class="table" width="100%" border="0" cellspacing="0" cellpadding="0">
    		  <tr>
        	<td style="padding-left:5px;" width="50%"><b>Character status:</b></td>
    	    <td rowspan="10" valign="top" align="center" style="padding-top:10px; padding-bottom:10px;"><b>::User picture::</b><br>
    	      ';
    	    ...
    	    <td style="padding-left:5px;">Race: '.$char_data[0]['race'].'</td></tr><tr>
  		    <td style="padding-left:5px;">Clan: '.$clan.'</td></tr><tr>
  		    <td style="padding-left:5px;">Health: '.$char_data[0]['cur_health'].' / '.$char_data[0]['max_health'].' </td></tr><tr>
  		    <td style="padding-left:5px;">Reiatsu: '.$char_data[0]['cur_rei'].' / '.$char_data[0]['max_rei'].'</td></tr><tr>';
